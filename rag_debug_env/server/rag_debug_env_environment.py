...
from src.server.corpus import _load_corpus
from src.server.fault_math import apply_faults
...
class RAGDebugEnvironment(Environment):
    ...
    def step(
        ...
        **kwargs: Any,
        ...
        # Auto-terminate on max steps; penalise timeout so the agent learns
        # to submit before the budget runs out rather than wandering.
        if self._state.step_count >= _MAX_STEPS and not self._done:
            self._done = True
            reward_value -= 0.25  # timeout penalty
    ...
    def _apply_action(self, action: RAGDebugAction) -> Optional[float]:
        ...
        if t == ActionType.ADJUST_CHUNK_SIZE:
            ...
            if query_id in ep_ids:
                ...
                if cols:
                    # Imperfect boost: a real rewrite improves but does not
                    # perfectly surface all relevant chunks. Use a deterministic
                    # per-(episode, query) RNG so replaying the same action gives
                    # the same result within an episode.
                    rw_rng = np.random.default_rng(
                        int(self._internal_state.episode_seed) ^ (int(query_id) * 31337)
                    )
                    # Relevant chunks get a moderate, slightly noisy boost
                    boosts = rw_rng.uniform(0.07, 0.13, size=len(cols)).astype(np.float32)
                    self._rewrite_boosts[row, cols] = boosts
                    # ~5% of irrelevant chunks also get a small false-positive boost
                    n_c = len(self._chunk_ids)
                    irrelevant = list(set(range(n_c)) - set(cols))
                    if irrelevant:
                        n_fp = max(1, n_c // 20)
                        fp_cols = rw_rng.choice(
                            irrelevant, size=min(n_fp, len(irrelevant)), replace=False
                        )
                        self._rewrite_boosts[row, fp_cols] = rw_rng.uniform(
                            0.02, 0.06, size=len(fp_cols)
                        ).astype(np.float32)
            self._recompute_S_faulted()
